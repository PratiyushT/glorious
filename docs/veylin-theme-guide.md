...
JavaScript progressively enhances filtering, sorting, pagination, and collection-to-collection changes through Shopify's Section Rendering API. The collection pills use the same interaction contract as filters: the shared themed loader appears immediately, the current product results dim, the selected Product grid animation introduces the replacement cards, and the view scrolls smoothly to the results. A collection change still fetches its destination header and product catalog together so those server-rendered surfaces can be committed atomically before the URL changes. Links and forms remain functional when JavaScript is unavailable, a request fails, or the browser cannot support the enhancement.

Each server-rendered replacement rebinds the new catalog root before another interaction can occur. Consecutive collection, filter, sort, and pagination changes therefore remain enhanced instead of alternating between animation and a full page load. Browser Back and Forward restore the server-rendered collection state without forcing the click-only results scroll.
