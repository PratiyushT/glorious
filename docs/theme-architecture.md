...
| Collection | `collection.json`: Collection header, Main collection | Contextual Collection title/image header composition with side-by-side, title-over-image, image-only, and title-only arrangements; static `_product-card` | Catalog controls/filter/sort/pagination, responsive collection media, and the shared product card family |
